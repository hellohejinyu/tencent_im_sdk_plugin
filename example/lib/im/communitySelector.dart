...
class CommunityGroupSelector extends StatefulWidget {
  final OnSelect onSelect;
  final bool switchSelectType;
  final List<String> value;
  const CommunityGroupSelector(
      {Key? key,
...
class GroupSelectorState extends State<CommunityGroupSelector> {
  @override
  void initState() {
    super.initState();
    setState(() {
  ...
  Future<List<V2TimGroupInfo>?> getJoinedCommunityList() async {
  ...
  Widget build(BuildContext context) {
    return ElevatedButton(
      onPressed: () async {
        List<V2TimGroupInfo>? fl = await getJoinedCommunityList();
        if (fl != null) {
          if (fl.isNotEmpty) {
            showDialog<void>(
              context: context,
